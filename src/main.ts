...
function cargarEventos() {
  return new Promise((resolve, reject) => {
    ...
    (window as any)[callbackName] = (data: Evento) => {
      resolve(data);
      document.body.removeChild(script);
      delete (window as Record<string, any>)[callbackName];
    };
    script.src = `https://script.google.com/macros/s/AKfycbx4odRCfNvLGkgkfN0Xg6kW7vNeD_l-efOaULp_uvF75cLsOwyXcg1LQtZPeRCvWCWC/exec?callback=${callbackName}`;
    script.onerror = () => {
...
async function iniciar() {
  try {
    ...
    console.log("EVENTOS", eventos);
    // Aquí puedes llamar a la función que renderiza los eventos en tu página
  } catch (err) {
